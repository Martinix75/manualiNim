...
proc livex*(win: QDialog; tab: Table[string, string]):Keep  {.raises: [].} =
    echo("Modal Win")
    let form = Keep()
    form.mlay_1 = QGridLayout.create()
    form.dlg = Qdialog.create(win)
    ...
    connect(form.msbx_1, onValueChanged, upDateTab, form)
    connect(form.mqcb_1, onStateChanged, upDateTab, form)
    connect(form.mqle_1, onTextChanged, upDateTab, form)
    connect(form.mqle_2, onTextChanged, upDateTab, form)
